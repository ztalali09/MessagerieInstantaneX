...
export function generateRSAKeyPair(): RSAKeyPair {
  ...
  // 512 bits is insecure but faster for JS. 1024 or 2048 might freeze the thread.
  const bitLength = 512; 
  ...
  // Simple JSON format for keys instead of complex PEM
...
export function encryptWithPublicKey(data: string, publicKeyStr: string): string {
  try {
    ...
    // Convert string to BigInt (simple encoding)
...
export function decryptWithPrivateKey(encryptedDataHex: string, privateKeyStr: string): string {
  try {
